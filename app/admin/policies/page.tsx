...
export default function AdminPoliciesPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      <AdminNavbar
        pageTitle="Academic Policy CMS Manager"
        pageSubtitle="Publish, update, and manage official regulations, grading scales, pass marks, and student conduct rules."
        showBack={true}
      />
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-3xl shadow-xs border border-slate-200/90">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <h2 className="text-base font-black text-slate-900">Official Regulations Handbook CMS</h2>
              <p className="text-xs text-slate-500 font-medium mt-0.5">
                School of Nursing, UCH • Real-time policy publication to student portals.
              </p>
            </div>
          </div>
          ...
          <div className="bg-white p-6 sm:p-8 rounded-3xl shadow-xs border border-slate-200/90 space-y-4">
            ...
            {loading ? (
              ...
              <div className="space-y-3 max-h-[500px] overflow-y-auto pr-1">
                {policies.map((p) => (
                  <div key={p.id} className="p-4 bg-slate-50 border border-slate-200 rounded-2xl space-y-1.5">
